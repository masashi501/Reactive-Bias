...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            embed_layer=PatchEmbed, norm_layer=None, act_layer=None, block_fn=Block ,num_reg=0):
        """
        Args:
        ...
        self.A_bias = nn.Parameter(torch.zeros(1, depth, num_heads, num_patches + self.num_tokens + num_reg, num_patches + self.num_tokens + num_reg)) # A_bias = Bias mode [patch, head, layer, token_num, token_num]
    ...
    def forward_features(self, x):
        ...
        if self.A_bias is not None:
            A_bias = self.A_bias.expand(x.shape[0],-1,-1,-1,-1) # A_bias = Bias mode [patch, head, layer, token_num, token_num]
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for blk in self.blocks:
                x = blk(x,A_bias[:,A]) # A_bias = Bias mode [patch, head, layer, token_num, token_num]
                A+=1
        x = self.norm(x)
        return x
